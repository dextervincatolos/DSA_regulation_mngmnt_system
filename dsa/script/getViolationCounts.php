...
$daily_query = "SELECT COUNT(*) AS count
                FROM violation_tbl
                WHERE DATE(created_at) = '$today' AND violation_status = 'Open'";
$daily_count = getCountFromQuery($connection, $daily_query);

$weekly_query = "SELECT COUNT(*) AS count
                 FROM violation_tbl
                 WHERE YEARWEEK(created_at, 0) = YEARWEEK('$today', 0)  AND violation_status = 'Open'";
$weekly_count = getCountFromQuery($connection, $weekly_query);

$monthly_query = "SELECT COUNT(*) AS count
                  FROM violation_tbl
                  WHERE DATE_FORMAT(created_at, '%Y-%m') = DATE_FORMAT(NOW(), '%Y-%m') AND violation_status = 'Open'";
