    ...
    //bar Chart------------------------------------------------
        ...
        function fetchDataAndInitializeGroupedBarChart() {
            fetch('script/fetchViolation.php')
                ...
                .then(data => {
                    console.log(data);
                    const departments = [...new Set(data.departmentViolations.map(d => d.course_name))];
                    ...
                    const datasets = policies.map((policy, index) => {
                        return {
                            ...
                            data: departments.map(department => {
                                const violation = data.departmentViolations.find(v => v.course_name === department && v.policy_title === policy);
                                return violation ? violation.count : 0;
                            })
                        };
                    });
